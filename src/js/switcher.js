...
if (jQuery.cookie) {
	if(!jQuery.isEmptyObject( jQuery.cookie() )) {
		deleteAllCookiesHTML = '<span className="rt-icon2-delete-outline" data-toggle="tooltip" data-placement="top" title="Clear All Styles (Page will be reloaded)"></span>';
	}
}
		
var switcherHTML = '<div id="switcher">';
		switcherHTML +=    '<div className="color_switcher_header"><h4>Styles Selector</h4>';
		switcherHTML +=    		'<span className="rt-icon2-brush"></span>';
		switcherHTML +=    		deleteAllCookiesHTML;
		switcherHTML +=    '</div>';
		switcherHTML +=    '<div id="switcher_color_scheme">';
		switcherHTML +=    		'<p>Color scheme:</p>';
		switcherHTML +=    		'<ul id="switcher-version" className="list-inline">';
		switcherHTML +=        		'<li className="active">';
		switcherHTML +=            		'<a href="#" className="light">Light</a>';
		switcherHTML +=        		'</li>';
		switcherHTML +=        		'<li>';
		switcherHTML +=            		'<a href="#" className="dark">Dark</a>';
		switcherHTML +=        		'</li>';
		switcherHTML +=    		'</ul>';
		switcherHTML +=    '</div>';
		switcherHTML +=    '<div id="switcher_layout">';
		switcherHTML +=    		'<p>Layout style:</p>';
		switcherHTML +=    		'<div className="checkbox checkbox-slider--b-flat">';
		switcherHTML +=    		    '<label>';
		switcherHTML +=    		        '<input type="checkbox" id="layout"> <span>Boxed</span>';
		switcherHTML +=    		    '</label>';
		switcherHTML +=    		'</div>';
		switcherHTML +=    		'<div className="checkbox checkbox-slider--b-flat">';
		switcherHTML +=    		    '<label id="boxed_margin" className="hidden">';
		switcherHTML +=    		        '<input type="checkbox"> <span>Vertical Margins</span>';
		switcherHTML +=    		    '</label>';		
		switcherHTML +=    		'</div>';
		switcherHTML +=		'</div>';
		switcherHTML +=    '<div id="switcher_patterns">';
		switcherHTML +=    		'<p className="for-toggle hidden">Boxed Patterns</p>';
		switcherHTML +=    		'<ul id="switcher-patterns" className="list-inline for-toggle hidden">';

		for (var i = 0; i < 12; i++) {
			switcherHTML +=    		    '<li>';
			switcherHTML +=    		        '<a href="#" className="pattern' + i + '" data-pattern="pattern' + i + '">';
		...
		//switcher toggle
        jQuery('#switcher span[className="rt-icon2-brush"]').on('click', function(){
            jQuery('#switcher').toggleClass('active');
        });
		
		//switcher reset all styles
        jQuery('#switcher span[className="rt-icon2-delete-outline"]').on('click', function(){
